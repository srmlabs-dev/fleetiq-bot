...
(function (global) {
  ...
  // Normalize ISO datetime strings to YYYY-MM-DD for date comparisons
  // '2026-05-01T05:00:00.000Z' → '2026-05-01'
  function _normDate(d) {
    if (!d) return '';
    if (typeof d === 'string' && d.length > 10 && d[10] === 'T') return d.slice(0, 10);
  ...
  function getWeekLoads() {
    ...
    return _get.loads().filter(x => _getWeekKey(_normDate(x.pickup || x.date || '')) === wk);
  }
  ...
  function getExportLoads() {
    ...
    return _get.loads().filter(x => {
      const d = _normDate(x.pickup || x.date || '');
      return d >= from && d <= to;
    });
  }
  ...
  function fmtDateRangeFromLoads(rLoads) {
    ...
    const dates = rLoads.map(x => _normDate(x.pickup || x.date || '')).filter(Boolean).sort();
      ...
      const rows  = loads.map((x,i) => {
        ...
        return `<tr style="border-bottom:1px solid #e2e8f0;background:${i%2===0?'#fff':'#f8fafc'}">
          <td style="padding:7px 10px;font-weight:600;font-size:12px">${_escHtml(x.loadId)}<br>
            <span style="font-size:10px;color:#94a3b8">${_normDate(x.pickup||'')}${x.delivery?' \u2192 '+_normDate(x.delivery||''):''}</span><br>
...
<html><head>
      ...
      return `<div class="item" style="border-left:3px solid ${borderColor}">
        <div class="item-inner">
          <div>
            ...
            <div class="muted">${_normDate(x.pickup||'')||''}${x.delivery?' → '+_normDate(x.delivery||''):''}</div>
  ...
  function renderStats() {
    ...
    if (period === 'week') {
      const wk = _getWeekKey(t);
      filtered = loads.filter(x => _getWeekKey(_normDate(x.pickup || x.date || '')) === wk);
    } else if (period === 'month') {
      const ym = t.slice(0, 7);
      filtered = loads.filter(x => _normDate(x.pickup || '').slice(0, 7) === ym);
    }
    ...
    const wkMap = {};
    loads.forEach(x => {
      const wk = _getWeekKey(_normDate(x.pickup || x.date || ''));
      if (!wk) return;
